...
def product_list(request):
    products = Product.objects.all().order_by('id') #получаем все детские товары с обязательной сортировкой для пагинатора
...
def product_detail(request, pk):
    product = get_object_or_404(Product, id=pk) #отображение информации о детском товаре с обработкой ошибки 404 по заданию
...
def cart_view(request):
    ...
    return render(request, 'shop/cart.html', {'cart': cart}) #отображение всех элементов корзины

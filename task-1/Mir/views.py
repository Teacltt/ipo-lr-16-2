...
def get_or_create_cart(user):
    # Возврат или инициализация личной корзины покупателя детских товаров
...
def detmir_home(request):
    ...
    if request.method == 'POST':
        # ТРЕБОВАНИЕ ЗАДАНИЯ 1: Только менеджер склада или администратор могут добавлять детские товары
...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.get_user()
            login(request, user) # Django генерирует сессионный токен и куку сессии
            return redirect('home')
    else:

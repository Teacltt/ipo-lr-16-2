...
from django.contrib.auth.decorators import login_required #импортируем декоратор ограничения доступа по заданию четвертого пункта
...
def product_list(request):
    ...
    return render(request, 'catalog.html', {
        ...
        'categories': categories,
        'manufacturers': manufacturers
    }) #открываем страницу списка товаров каталога детского мира


def product_detail(request, pk):
    product = get_object_or_404(Product, id=pk) #отображение информации о детском работе с обработкой ошибки 404 по заданию
    return render(request, 'product_detail.html', {'product': product}) #открываем страницу детальной информации о детском товаре


@login_required #ограничиваем доступ к добавлению в корзину только для авторизованных пользователей по заданию четвертого пункта
def add_to_cart(request, product_id):
    cart = get_or_create_test_cart(request.user) #получаем личную корзину текущего вошедшего пользователя по заданию четвертого пункта
...
@login_required #ограничиваем доступ к изменению количества только для авторизованных пользователей по заданию четвертого пункта
def update_cart(request, item_id):
    ...
    cart = get_or_create_test_cart(request.user) #получаем текущую корзину детского мира со всеми элементами
    return render(request, 'cart.html', {'cart': cart, 'error_message': error_message}) #открываем страницу корзины с переданной ошибкой


@login_required #ограничиваем доступ к удалению из корзины только для авторизованных пользователей по заданию четвертого пункта
...
@login_required #ограничиваем доступ к просмотру корзины только для авторизованных пользователей по заданию четвертого пункта
def cart_view(request):
    cart = get_or_create_test_cart(request.user) #получаем корзину со всеми элементами для отображения по заданию четвертого пункта
    return render(request, 'cart.html', {'cart': cart}) #отображение всех элементов корзины с общей стоимостью по заданию четвертого пункта
